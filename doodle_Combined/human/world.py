...
class World:
    ...
    def update(self, camera):
        ...
        for p in self.platforms:
            if camera.apply(p).y > HUYWIN or camera.apply(p).y < -400:
                self.platforms.remove(p)

        for b in self.bonuses:
            if camera.apply(b).y > HUYWIN or camera.apply(b).y < -400:
    ...
    def generate(self, camera):
        if len(self.platforms) > 0:
            ...
            # 选择x和y坐标
            x = random.randint(0, HUXWIN - self.widthPlatform)
            ...
            self.platforms = [block.Block(self.startX - 50,
                                    self.startY + 50,
                                    self.widthPlatform,
                                    self.heightPlatform,
