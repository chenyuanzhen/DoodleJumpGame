...
class World:
    ...
    def generate(self, camera):
        if len(self.platforms) > 0:
            ...
            breack, randomBonus, randomCanJump = random.randint(0, self.platformChanceBroken), random.randint(0,
                                                                                                              self.bonusSpawnChance), random.randint(
                0, 6)
...
class Camera(object):
    ...
    def update(self, target):
        ...
        # 如果玩家将要移动的地方 满足一定条件，缓慢向上移动
        # apply(target) :玩家将要移动的地方
...
while loopGame:
    ...
    # draw platforms and spring onto window
